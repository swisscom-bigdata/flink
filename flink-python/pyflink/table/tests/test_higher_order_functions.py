...
class HigherOrderFunctionTests(object):
    ...
    actually execute the resulting job so that the Python lambda adapters (``_LambdaFunction``,
    ``_LambdaBiFunction``, ``_LambdaTriFunction``), the outer-column capture lifting and the
