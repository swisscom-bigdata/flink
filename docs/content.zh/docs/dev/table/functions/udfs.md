...
* `getExpectedLambdaParameterTypes(callContext, argumentPos)` 根据同一次调用中其余（此时已解析完成的）参数，
  ...
  返回的列表必须包含一个、两个或三个类型；其它数量在两种接口上都会被 `ValidationException` 拒绝。
...
lambda，即用 `FunctionData`、`BiFunctionData` 和 `TriFunctionData` 代替 `Function`、`BiFunction` 和
`TriFunction`。它与其它参数的表示一样声明在参数上，在 `inferInputTypes(...)` 中对该参数类型做 bridge 即可：
...
* **参数与结果。** `apply` 的参数可以为 `null`，返回值也可以为 `null`（即 SQL `NULL`）。使用外部类的
  `Function`、`BiFunction` 或 `TriFunction` 交换的是外部值；使用内部类的 `FunctionData`、
  `BiFunctionData` 或 `TriFunctionData` 返回的可能是由求值器持有的可变内存，下一次应用会复用它。
  如果函数需要跨多次应用保留或缓存这样的内部结果，必须先做深拷贝。
