...
* `getExpectedLambdaParameterTypes(callContext, argumentPos)` binds the parameter types of the lambda
  ...
  returned list must contain between zero and four types, matching the arities the `FunctionN`
  family covers; any other size is rejected with a `ValidationException` on both surfaces. An empty
  list means a lambda that takes no parameters, not a declined binding — that is what the empty
  optional is for.
...
| Lambda parameters | Object handed to `eval` | Also accepted |
|---|---|---|
...
A lambda may take up to four parameters, because that is how far the `FunctionN` family reaches.
`Function1`, `Function2` and `Function3` merely extend `Function`, `BiFunction` and `TriFunction`,
so a function may declare either spelling for those arities. `Function0` is the other way round: it
extends `java.util.function.Supplier` and declares `apply()`, because the generated object's method
is always `apply`. A `Supplier` parameter therefore works too, but only `Function0` can be named in
`bridgedTo(...)`.
...
`FunctionData0` to `FunctionData4` instead of `Function0` to `Function4`. It is declared on the
argument, next to the representation of every other argument, by bridging the argument's type in
`inferInputTypes(...)`:
...
* **Arguments and result.** `apply` may be called with `null` arguments and may return `null`
  (SQL `NULL`). An external `FunctionN` exchanges external values. An internal `FunctionDataN` may
  return a mutable value backed by evaluator-owned memory that the next application reuses. A
  function that retains or buffers such an internal result across another application must
  deep-copy it first.
