...
import org.apache.flink.table.planner.functions.utils.{HigherOrderFunctionUtil, ScalarSqlFunction, TableSqlFunction}
...
class ExprCodeGenerator(
  ...
  override def visitCall(call: RexCall): GeneratedExpression = {
    ...
    val operands = call.getOperands.zipWithIndex.map {
      ...
      // A lambda operand is compile-time only: it is consumed during specialization via
      // CallContext.getLambdaArgument and never evaluated as a runtime value. Emit a placeholder
      // that BridgingFunctionGenUtil filters out.
      case (o @ _, _) if isLambdaOperand(o) =>
        GeneratedExpression(
          "null",
          ALWAYS_NULL,
          NO_CODE,
          FlinkTypeFactory.toLogicalType(resolveLambda(o).getType))
  ...
  override def visitLambda(lambda: RexLambda): GeneratedExpression =
    throw new CodeGenException("Lambdas are only supported as arguments of higher-order functions.")

  // A lambda body is never generated by this visitor: it is compiled into a separate expression
  // evaluator, where the lambda's parameters are ordinary input fields (see
  // BridgingFunctionGenUtil.generateLambdaFunctionObject). A RexLambdaRef reaching here therefore
  // means a body escaped its lambda.
   ...
   */
  private[codegen] def resolveLambda(node: RexNode): RexLambda = node match {
  ...
  /**
   * Whether the given operand is (or, inside a [[RexProgram]], references) a [[RexLambda]]. Such
   * operands are compile-time only for user-defined higher-order functions.
   */
  private def isLambdaOperand(node: RexNode): Boolean = node match {
    case _: RexLambda => true
    case localRef: RexLocalRef if rexProgram != null =>
      isLambdaOperand(rexProgram.getExprList.get(localRef.getIndex))
    case _ => false
  }
